...
const StudioInterface: React.FC = () => {
  ...
  const handleAddMapPoolElement = () => { addStudioElement("MapPool"); };
      ...
      <main style={{ flexGrow: 1, padding: '1rem', position: 'relative', overflow: 'hidden' }} onClick={(e) => { if (e.target === e.currentTarget) { setSelectedElementId(null); } }}>
        ...
        >
          ...
          {activeLayout.map((element: StudioElement, index: number) => { // Added index here
            ...
            else if (element.type === "MapPool") { content = <MapPoolElement id={element.id} position={element.position} size={element.size} scale={element.scale} playerId={element.playerId} pairId={element.pairId} isPairMaster={element.isPairMaster} />; }
