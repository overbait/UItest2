...
import {
  ...
  SavedPreset,
  StudioElement,
  SavedStudioLayout // Make sure this is imported and defined in types/draft.ts
} from '../types/draft';
...
const initialCombinedState: CombinedDraftState = {
  // ... (all existing initial state properties remain the same)
  ...
  studioLayout: [], savedStudioLayouts: [],
};
...
const transformRawDataToSingleDraft = ( raw: Aoe2cmRawDraftData, draftType: 'civ' | 'map' ): Partial<SingleDraftData> => {
  ...
  const output: Partial<SingleDraftData> = {
    id: raw.id || raw.draftId || 'unknown-id', hostName, guestName,
    civPicksHost: [], civBansHost: [], civPicksGuest: [], civBansGuest: [], mapPicksHost: [],
    mapBansHost: [], mapPicksGuest: [], mapBansGuest: [], mapPicksGlobal: [], mapBansGlobal: [],
  };
  ...
  if (raw.preset?.turns && typeof raw.nextAction === 'number') {
    if (raw.nextAction >= raw.preset.turns.length) draftStatus = 'completed';
    else {
  ...
  } else if (raw.status) draftStatus = raw.status.toLowerCase() as SingleDraftData['status'];
  else if (raw.ongoing === false) draftStatus = 'completed';
  else if (raw.ongoing === true) draftStatus = 'inProgress';
      ...
      (set, get) => ({
        ...
        _resetCurrentSessionState: () => {
          set({
            ...
            studioLayout: [],
            savedStudioLayouts: get().savedStudioLayouts,
          });
        },
          ...
          if (activePresetId) {
            ...
            if (presetIndex !== -1) {
              const updatedPreset: SavedPreset = {
                ...savedPresets[presetIndex], hostName, guestName, scores: { ...scores }, civDraftId, mapDraftId,
                boxSeriesFormat, boxSeriesGames: JSON.parse(JSON.stringify(boxSeriesGames)),
              };
              const newSavedPresets = [...savedPresets]; newSavedPresets[presetIndex] = updatedPreset;
          ...
          try {
            if (url.startsWith('http://') || url.startsWith('https://')) {
              ...
              if (urlObj.hostname.includes('aoe2cm.net')) { 
                const pathMatch = /\/draft\/([a-zA-Z0-9]+)/.exec(urlObj.pathname); if (pathMatch && pathMatch[1]) return pathMatch[1];
                const observerPathMatch = /\/observer\/([a-zA-Z0-9]+)/.exec(urlObj.pathname); if (observerPathMatch && observerPathMatch[1]) return observerPathMatch[1];
              }
              const pathSegments = urlObj.pathname.split('/'); const potentialId = pathSegments.pop() || pathSegments.pop();
              if (potentialId && /^[a-zA-Z0-9_-]+$/.test(potentialId) && potentialId.length > 3) return potentialId;
              const draftIdParam = urlObj.searchParams.get('draftId') || urlObj.searchParams.get('id'); if (draftIdParam) return draftIdParam;
            }
            if (/^[a-zA-Z0-9_-]+$/.test(url) && url.length > 3) return url;
            return null;
          } catch (error) {
            if (/^[a-zA-Z0-9_-]+$/.test(url) && url.length > 3) return url;
            return null;
        ...
        connectToDraft: async (draftIdOrUrl: string, draftType: 'civ' | 'map') => {
          ...
          const currentActivePresetId = get().activePresetId; const savedPresetsArray = get().savedPresets;
          const activePreset = currentActivePresetId ? savedPresetsArray.find(p => p.id === currentActivePresetId) : null;
          if (activePreset) {
            if ((draftType === 'civ' && activePreset.civDraftId !== extractedId) || (draftType === 'map' && activePreset.mapDraftId !== extractedId)) set({ activePresetId: null });
          ...
          try {
            ...
            if (get().activePresetId === null && !get().boxSeriesFormat) { 
                if (presetName.includes('bo1')) detectedFormat = 'bo1'; else if (presetName.includes('bo3')) detectedFormat = 'bo3';
                else if (presetName.includes('bo5')) detectedFormat = 'bo5'; else if (presetName.includes('bo7')) detectedFormat = 'bo7';
            }
            if (detectedFormat && get().boxSeriesFormat !== detectedFormat) get().setBoxSeriesFormat(detectedFormat);
            if (draftType === 'civ') {
              set(state => ({
                hostName: processedData.hostName || state.hostName, guestName: processedData.guestName || state.guestName,
                civPicksHost: processedData.civPicksHost || [], civBansHost: processedData.civBansHost || [],
                civPicksGuest: processedData.civPicksGuest || [], civBansGuest: processedData.civBansGuest || [],
                civDraftStatus: 'connected', isLoadingCivDraft: false, civDraftError: null,
                boxSeriesGames: state.boxSeriesFormat ? state.boxSeriesGames.map((game, index) => ({ ...game,
              ...
              set(state => {
                const combinedMapPicks = Array.from(new Set([...(processedData.mapPicksHost || []), ...(processedData.mapPicksGuest || []), ...(processedData.mapPicksGlobal || []) ]));
                return {
                  ...
                  mapPicksHost: processedData.mapPicksHost || [], mapBansHost: processedData.mapBansHost || [],
                  mapPicksGuest: processedData.mapPicksGuest || [], mapBansGuest: processedData.mapBansGuest || [],
                  mapPicksGlobal: processedData.mapPicksGlobal || [], mapBansGlobal: processedData.mapBansGlobal || [],
                  mapDraftStatus: 'connected', isLoadingMapDraft: false, mapDraftError: null,
                  boxSeriesGames: state.boxSeriesFormat ? state.boxSeriesGames.map((game, index) => ({ ...game, map: combinedMapPicks[index] || game.map || null, })) : [],
                };
              });
            }
            get()._updateActivePresetIfNeeded(); return true;
          } catch (error) {
            let errorMessage = `Failed to fetch or process ${draftType} draft data.`;
            if (axios.isAxiosError(error)) errorMessage += ` Server: ${error.response?.status || 'N/A'}: ${error.message}`; else errorMessage += ` Error: ${(error as Error).message}`;
        ...
        disconnectDraft: (draftType: 'civ' | 'map') => {
          if (draftType === 'civ') {
            set({ civDraftId: null, civDraftStatus: 'disconnected', civDraftError: null, isLoadingCivDraft: false, civPicksHost: [], civBansHost: [], civPicksGuest: [], civBansGuest: [], hostName: get().mapDraftId ? get().hostName : initialPlayerNameHost, guestName: get().mapDraftId ? get().guestName : initialPlayerNameGuest, boxSeriesGames: get().boxSeriesGames.map(game => ({ ...game, hostCiv: null, guestCiv: null })), activePresetId: null, });
          } else { 
            set({ mapDraftId: null, mapDraftStatus: 'disconnected', mapDraftError: null, isLoadingMapDraft: false, mapPicksHost: [], mapBansHost: [], mapPicksGuest: [], mapBansGuest: [], mapPicksGlobal: [], mapBansGlobal: [], boxSeriesGames: get().boxSeriesGames.map(game => ({ ...game, map: null })), activePresetId: null, });
          }
          if (!get().civDraftId && !get().mapDraftId) set({ boxSeriesFormat: null, boxSeriesGames: [], activePresetId: null });
        },
        reconnectDraft: async (draftType: 'civ' | 'map') => {
          const idToReconnect = draftType === 'civ' ? get().civDraftId : get().mapDraftId;
          if (!idToReconnect) { const errorMsg = `No ${draftType} draft ID to reconnect.`; if (draftType === 'civ') set({ civDraftError: errorMsg }); else set({ mapDraftError: errorMsg }); return false; }
        ...
        saveCurrentAsPreset: (name?: string) => {
          ...
          const presetData: SavedPreset = { id: presetIdToUse, name: presetName, civDraftId, mapDraftId, hostName, guestName, scores: { ...scores }, boxSeriesFormat, boxSeriesGames: JSON.parse(JSON.stringify(boxSeriesGames)), };
          if (existingPresetIndex !== -1) { const updatedPresets = [...savedPresets]; updatedPresets[existingPresetIndex] = presetData; set({ savedPresets: updatedPresets, activePresetId: presetData.id });
          } else set({ savedPresets: [...savedPresets, presetData], activePresetId: presetData.id });
        },
        ...
        deletePreset: (presetId: string) => {
          ...
          if (currentActiveId === presetId) get()._resetCurrentSessionState();
        },
        updatePresetName: (presetId: string, newName: string) => {
          set(state => ({ savedPresets: state.savedPresets.map(p => p.id === presetId ? { ...p, name: newName } : p), activePresetId: state.activePresetId === presetId ? presetId : state.activePresetId, })); get()._updateActivePresetIfNeeded();
        },
        setBoxSeriesFormat: (format) => {
          let numGames = 0; if (format === 'bo1') numGames = 1; else if (format === 'bo3') numGames = 3; else if (format === 'bo5') numGames = 5; else if (format === 'bo7') numGames = 7;
          let newGames = Array(numGames).fill(null).map(() => ({ map: null, hostCiv: null, guestCiv: null, winner: null }));
          const state = get(); if (numGames > 0) { const combinedMapPicks = Array.from(new Set([...state.mapPicksHost, ...state.mapPicksGuest, ...state.mapPicksGlobal])).filter(Boolean); newGames = newGames.map((_game, index) => ({ map: combinedMapPicks[index] || null, hostCiv: state.civPicksHost[index] || null, guestCiv: state.civPicksGuest[index] || null, winner: null, })); }
          set({ boxSeriesFormat: format, boxSeriesGames: newGames }); get()._updateActivePresetIfNeeded();
        },
        updateBoxSeriesGame: (gameIndex, field, value) => {
          set(state => { const newGames = [...state.boxSeriesGames]; if (newGames[gameIndex]) { newGames[gameIndex] = { ...newGames[gameIndex], [field]: value, winner: null }; return { boxSeriesGames: newGames }; } return state; }); get()._updateActivePresetIfNeeded();
        },
        setGameWinner: (gameIndex: number, winningPlayer: 'host' | 'guest' | null) => {
          set(state => { const newGames = [...state.boxSeriesGames]; if (newGames[gameIndex]) { if (newGames[gameIndex].winner === winningPlayer) newGames[gameIndex] = { ...newGames[gameIndex], winner: null }; else newGames[gameIndex] = { ...newGames[gameIndex], winner: winningPlayer }; } let hostScore = 0; let guestScore = 0; newGames.forEach(game => { if (game.winner === 'host') hostScore++; else if (game.winner === 'guest') guestScore++; }); return { boxSeriesGames: newGames, scores: { host: hostScore, guest: guestScore }}; }); get()._updateActivePresetIfNeeded();
        },

        addStudioElement: (elementType: string) => {
          set(state => { const newElement: StudioElement = { id: Date.now().toString(), type: elementType, position: { x: 10, y: 10 + state.studioLayout.length * 50 }, size: { width: 250, height: 40 }, }; return { studioLayout: [...state.studioLayout, newElement] }; });
        },
        ...
        saveCurrentStudioLayout: (name: string) => {
          set(state => {
            const newLayoutPreset: SavedStudioLayout = {
              ...
              name,
              layout: JSON.parse(JSON.stringify(state.studioLayout)), // Deep copy
            };
            return {
        ...
        loadStudioLayout: (layoutId: string) => {
          set(state => {
            ...
            if (layoutToLoad) {
              return { studioLayout: JSON.parse(JSON.stringify(layoutToLoad.layout)) }; // Deep copy
            }
            return state;
          });
        },
        ...
        updateStudioLayoutName: (layoutId: string, newName: string) => {
          set(state => ({
            savedStudioLayouts: state.savedStudioLayouts.map(l =>
